...
% Manual Z-stack split (NO ScanImage metadata) + per-slice pipeline:
%   slice tif -> dark -> rigid MC -> cpSAM -> dFF (toss first 30 frames)
%
% Output:
...
%% ========================= USER INPUTS =========================
zstackTif = "C:\Users\Admin\Desktop\260210_chat_soma_g8s\roi1_-1050_400_z-25-115_IO_00001.tif";

% Z definition (MANUAL)
ZStart    = 0;
ZStep     = 10;  
NumSlices = 10;     %11     % z = 0,20,...,400

% Frames per slice (MANUAL)
FramesPerSlice = 3000;   

% If acquisition is slice blocks (all frames of z0, then all frames of z20, ...)
SliceOrder = "block";    % "block" | "interleaved"

% dFF settings
TossFrames = 30;         % toss first 30 frames BEFORE dFF
...
% Manual Z-stack splitter + pipeline runner (NO ScanImage metadata required).
% Handles multi-saved-channel TIFFs by selecting one KeepChannelID.
%
% Assumes TIFF time ordering is either:
%   - "block": all frames of z1 then all frames of z2 ...
%   - "interleaved": timepoints cycle through z slices: tp1=z1, tp2=z2,... repeat
%
% IMPORTANT:
%   If multiple channels are saved (e.g. channelSave=[1;3]), ScanImage stores
%   TIFF directories interleaved by channel per timepoint.
%   This function indexes TIMEPOINTS, then maps to TIFF directory by:
%       dirIdx = (tp-1)*nSaved + kWithin
...
p.addParameter("ZStep",  20, @(x)isnumeric(x)&&isscalar(x)&&x~=0);   % allow negative
...
% ---- FIX: channel handling params (were missing) ----
...
% --- detect channels saved in file ---
if pr.AutoDetectChannels
    savedCh = scanimage_saved_channels_from_tif(zstackTif);  % e.g. [1 3]
else
    savedCh = pr.KeepChannelID; % fallback
end

savedCh = savedCh(:).';
nSaved = numel(savedCh);
if nSaved < 1, nSaved = 1; savedCh = 1; end

% which position in the interleave corresponds to KeepChannelID?
kWithin = find(savedCh == pr.KeepChannelID, 1, "first");
if isempty(kWithin)
    % if requested channel isn't saved, fall back to first saved
    kWithin = 1;
end

% --- timepoints accounting ---
assert(mod(nFrames, nSaved) == 0, ...
    "TIFF has %d directories but saved-channels=%d -> not divisible.", nFrames, nSaved);

nTimepointsTotal   = nFrames / nSaved;
expectedTimepoints = pr.NumSlices * pr.FramesPerSlice;

assert(nTimepointsTotal == expectedTimepoints, ...
    "Timepoint count mismatch.\nTIFF has %d timepoints (= %d dirs / %d savedCh), but NumSlices*FramesPerSlice = %d*%d = %d.", ...
    nTimepointsTotal, nFrames, nSaved, pr.NumSlices, pr.FramesPerSlice, expectedTimepoints);
...
% store in idx (DON'T overwrite later)
idx = struct();
...
idx.keepChannelID    = pr.KeepChannelID;
idx.kWithin          = kWithin;
idx.nSavedChannels   = nSaved;

idx.slices = repmat(struct( ...
    "z_um",[], "subdir","", "timepoints",[], "tiff_dirs",[], "slice_tif","", ...
...
for s = 1:pr.NumSlices
    ...
    sliceTif = fullfile(subdir, sprintf("%s_z%d.tif", base, round(z)));

    % ---- FIX: compute TIMEPOINT indices, not TIFF directory indices ----
    if sliceOrder == "block"
        tpStart = (s-1)*pr.FramesPerSlice + 1;
        tpEnd   = s*pr.FramesPerSlice;
        tpIdx   = tpStart:tpEnd;
    ...
    % ---- FIX: map TIMEPOINT -> TIFF directory for the selected channel ----
    dirIdx = (tpIdx-1)*nSaved + kWithin;   % 1-based TIFF directories

    % write slice tif (uint16, LZW)
    if exist(sliceTif, "file"), delete(sliceTif); end
    for k = 1:numel(dirIdx)
        tIn.setDirectory(dirIdx(k));
        fr = tIn.read();
    ...
    % run getSAM inside the slice folder
    ...
    fprintf("[slice %02d/%02d | z=%d] wrote %d frames (ch=%d) -> %s\n", ...
        s, pr.NumSlices, round(z), numel(dirIdx), savedCh(kWithin), subdir);
end
...
if pr.SaveDiagnostics
    ...
    if exist("Fhist", "file") == 2
        ...
        % Call Fhist (it likely creates its own figure)
        try
...
%% save dark tif + info
% --- saveastiff hates string paths: force char + delete first ---
...
if mc_tif == "" || ~isfile(mc_tif)
    ...
    if isfile(mc_guess)
        ...
        % last resort: newest *_minusDark*MC*.tif in this folder
...
%stackDFF_simple(dFF, pr.FPS, pr.TraceGain, pr.MinOffset, pr.OffsetMult);
...
% Same look as your make_selROI_overlay_and_stackDFF stack plot:
% - median baseline removal per trace
% - multiply by gain (visual)
% - spacing from robust scale
% - 20% dFF scale bar (bar height scales with gain)
    ...
    for i = 1:N
        ...
        % trace index text at left (matches your "good" script vibe)
...
% Returns full path to newest file matching pattern in folderPath, else "".
...
function ch = scanimage_saved_channels_from_tif(tifPath)
% Returns vector of saved channel IDs (e.g., [1 2]) inferred from ScanImage metadata.
% Falls back to 1 if not found.

info = imfinfo(tifPath);
meta = string(info(1).Software) + newline;

meta = string(meta);
ch = parse_scanimage_channels(meta, "SI.hChannels.channelSave");
if isempty(ch)
    ch = parse_scanimage_channels(meta, "SI.hChannels.channelsActive");
end
if isempty(ch)
    ch = 1; % safest fallback
end
end

function ch = parse_scanimage_channels(metaStr, key)
% Parse a ScanImage metadata line like:
%   SI.hChannels.channelSave = 3
% or SI.hChannels.channelSave = [1 2]
% or SI.hChannels.channelSave = [true false true false]
% Returns channel IDs (1..4 typically). Empty if not found.

ch = [];

% Grab the RHS for the requested key
pat = key + "\s*=\s*([^\r\n]+)";
tok = regexp(metaStr, pat, "tokens", "once");
if isempty(tok), return; end
rhs = strtrim(string(tok{1}));

% Case A: scalar integer bitmask (e.g. "3")
if ~isempty(regexp(rhs, "^\d+$", "once"))
    m = str2double(rhs);
    if isfinite(m)
        % assume up to 4 channels unless metadata says otherwise
        nMax = 4;
        ch = find(bitget(uint32(m), 1:nMax));
        return;
    end
end

% Case B: bracketed list of numbers (e.g. "[1 2]" or "1 2")
nums = regexp(rhs, "[-+]?\d+\.?\d*", "match");
if ~isempty(nums)
    v = str2double(string(nums));
    v = v(isfinite(v));
    if ~isempty(v)
        % If it looks like explicit channel IDs (1..4), use them
        if all(v >= 1 & v <= 16) && all(mod(v,1)==0) && numel(unique(v)) == numel(v)
            ch = unique(v(:).', "stable");
            return;
        end
    end
...
% Case C: logical list (e.g. "[true false true false]")
