% append_0824_to_registry_260825.m
% -----------------------------------------------------------------------
...
sessionRel  = fullfile('ChAT', '0826');         % the session to append
scanName    = 'ChAT';                            % genotype for labels/keys
recDate     = '0826';
...
fprintf('\n=========== append_0824_to_registry_260825 ===========\n');
fprintf('registry : %s\n', regFile);
fprintf('session  : %s\n', sessionDir);
...
if ~isempty(already)
    error('append_0824:alreadyPresent', ...
...
if ~isempty(bad)
    ...
    error('append_0824:consistency','refusing to write the registry');
end
...
out.append_log = struct('session', sessionRel, 'appendedOn', '2026-08-25', ...
    'by', 'append_0824_to_registry_260825.m', ...
