...
outDir   = fullfile(rootPath, 'polar_coh_vs_rayleigh_260808');
% NEW output folder on purpose: the 260729 run is the reference figure and its
% .mat is what every downstream script recycles, so it must not be overwritten.
% This run differs from it in ONE way -- Sst\0806 and Sst\0807 were copied into
% the archive on 2026-08-08, so the 'Sst' scan now also finds those 24 recordings.
% They have no cell_link entry, so each of their ROIs counts as its own cell.
...
if showCoherencePanel
    ...
    ttlB = ['r = log Z      dot = MEAN direction (RY.th),  arc = IQR' newline ...
...
% per-group count of cells at logZ >= 2, shown beside n in the legend
grpOfCell_leg = cellfun(@(s) strtok(s,'/'), cellLabels(:), 'uni', 0);
n_z2_group = arrayfun(@(g) nnz(strcmp(grpOfCell_leg, groups{g}) & RY.logZ(:) >= 2), 1:numel(groups));
add_group_legend(fig, axB, groups, group_colors, n_per_group, n_z2_group);
...
% angle and so place at the WRONG phase.
    ...
    for k = 1:numel(C.logZ)
        ci = C.colorIdx(k);
        if isnan(C.th(k)) || ci<1 || ci>size(group_colors,1), continue; end
        if ~arcMask(k), continue; end
        r   = max(C.logZ(k), 0);
        col = group_colors(ci,:);
        if ci == 1, col = [0 0 0]; end
        arc = linspace(C.th(k)+C.q25(k), C.th(k)+C.q75(k), 40);
        %% ANGLE = RY.th (mean direction) as of 2026-08-11, was RY.thMed.
        %% CAVEAT: q25/q75 are residuals about the MEDIAN, so the arc is the
        %% median-centred IQR translated onto the mean. Where th and thMed
        %% disagree (>30 deg for 61 of 445 cells) the arc is shifted with the
        %% dot rather than recomputed -- the event phases are not stored.
    ...
    for k = 1:numel(C.logZ)
        ...
        if ci == 1     % IO: hollow black circle, matching panel A
            polarplot(ax, C.th(k), r, 'o', 'MarkerFaceColor','none', ...
                      'MarkerEdgeColor','k', 'MarkerSize',6, 'LineWidth',0.8);
        else
            polarplot(ax, C.th(k), r, 'o', 'MarkerFaceColor',col, ...
...
function add_group_legend(fig, refAx, groups, group_colors, n_per_group, n_z2_group)
% Color-key legend showing group -> hue + ROI count.
    ...
    for k = 1:N
        ...
        text(legAx, 0.25, y(k), sprintf('%s (n=%d, logZ>=2: %d)', groups{k}, n_per_group(k), n_z2_group(k)), ...
