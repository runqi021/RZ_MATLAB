% ventral_map_selected_260816.m   (copy of Ventral_surface_ROI_on_cartoon_260808)
% -----------------------------------------------------------------------
...
fprintf('  drawn on the right: %d of %d cells (IO dropped, Sert significant-only)\n', ...
        nDraw, numel(SEL.R));
...
for sg = 1:numel(scan_dirs)
    ...
    for hh = 1:numel(sam_hits)
        ...
        F(end+1) = struct('fov',fov,'gname',gname,'avgPath',avgPath, ...
                          ...
                          'drawPatch',~cfgEx,'cfgEx',cfgEx); %#ok<SAGROW>
    end
end
...
% COLOUR BY GROUP, not by phase. Same table as the population-average overlays --
% yellow-green Vglut2 pre-I, dark green Vglut2 I, light blue Vgat I, dark blue
% Vgat tonic -- so a trace in that figure and a dot here are the same cells.
% Every drawn cell is a FILLED dot in its group colour. Significance is no longer
% encoded in the marker -- it survives only as the Sert restriction and as the
% white outlines on the anatomy, and is still in the CSV.
DISP = group_display_colors_260816();
dispName = strings(nSel,1);
for i = 1:nSel
    dispName(i) = string(selGroup(selIds(i))) + " " + string(selClass(selIds(i)));
end
hG = gobjects(size(DISP,1),1); lG = strings(size(DISP,1),1);
for gi = 1:size(DISP,1)
    m = find(dispName == string(DISP{gi,1}));
    if isempty(m), continue; end
    col = DISP{gi,2};
    scatter(ax_p, rx(m), ry(m), 46, col, 'filled', ...
            'MarkerEdgeColor','k','LineWidth',0.5);
    hG(gi) = scatter(ax_p, NaN, NaN, 46, col, 'filled', ...
                     'MarkerEdgeColor','k','LineWidth',0.5);
    lG(gi) = sprintf('%s (n=%d)', DISP{gi,1}, numel(m));
end
...
fprintf('withheld on purpose: %d IO, %d non-significant Sert\n', ...
        nnz(string({SEL.R.group}) == "IO"), ...
        nnz(string({SEL.R.group}) == "Sert") - nnz(arrayfun(@(r) ...
            string(r.group)=="Sert" && r.logZ >= sel_critZ, SEL.R)));
ie = find([F.cfgEx]);
if ~isempty(ie)
...
title(ax_p, sprintf(['left: anatomy, %d FOVs / %d sites   |   right: %d cells ' ...
      'coloured by group.  IO not shown; Sert significant only (%s)'], ...
      nnz(~[F.cfgEx]), nnz([F.drawPatch]), nSel, critName));
...
exportgraphics(figOut, fullfile(outDir,'ventral_map_selected.png'), ...
               'Resolution',300,'BackgroundColor','white');
exportgraphics(figOut, fullfile(outDir,'ventral_map_selected.pdf'), ...
...
title(ax_p, sprintf(['zoom: %d cells coloured by group ' ...
      '(IO not shown, Sert significant only)'], nSel));
exportgraphics(figOut, fullfile(outDir,'ventral_map_selected_zoom.png'), ...
               'Resolution',400,'BackgroundColor','white');
exportgraphics(figOut, fullfile(outDir,'ventral_map_selected_zoom.pdf'), ...
               'ContentType','vector','BackgroundColor','white');
...
if isgraphics(ax_p), xlim(ax_p, xl0); ylim(ax_p, yl0); end
...
writetable(Tm, fullfile(outDir,'ventral_map_selected_cells.csv'));
save(fullfile(outDir,'ventral_map_selected_data.mat'), 'R','F','Tm','xd','yd', ...
