% whisk_LR_integral_breath_trough_quick_RZ.m
%
% Quick population xcorr:
%   int(L-R) vs breath
%
% Same preprocessing as current working script:
...
%   Integral: moving integral of L-R over 50 ms
%
% Lag convention:
%   corr( int(L-R)(t+lag), breath(t) )
%   lag > 0: breath leads int(L-R)
%   lag < 0: int(L-R) leads breath
%
% This script reports the TROUGH:
%   trough r = minimum population mean correlation

clear; clc;

take_abs = false;   % keep signed int(L-R), do NOT abs
...
for ai = 1:numel(ad)
    ...
    for kk = 1:numel(rr)
        ...
        mask = m & isfinite(int_LR) & isfinite(brw_bp1_20);
...
plot_xcorr_trough_panel(ax, xc_lr, lags_t, [0.2 0.2 0.2], ...
    'int(L-R) vs breath', ...
...
function plot_xcorr_trough_panel(ax, mat, lags_t, col, ttl, xlab, ylab)
    ...
    plot(ax, lags_t(tri), trv, 'ko', ...
        'MarkerFaceColor','k', ...
        'MarkerSize',6);

    title(ax, sprintf('%s\ntrough r=%.3f @ %.0f ms, n=%d', ...
        ttl, trv, 1000*lags_t(tri), n), ...
        'Interpreter','none');

    fprintf('%s: trough r=%.4f @ %.0f ms, n=%d\n', ...
        ttl, trv, 1000*lags_t(tri), n);
