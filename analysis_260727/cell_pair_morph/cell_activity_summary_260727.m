function cell_activity_summary_260727()
%% cell_activity_summary_260727
...
outSub      = fullfile('analysis_260727','cell_activity_summary');   % all 260727 output under one folder
...
if isfile(cp)
    ...
    if isfield(C.curated,'srcCellId') && numel(C.curated.srcCellId) == numel(g)
        % Curation was PORTED from a larger set: inherit that set's cell ids so the
        % same neuron carries the same number in baseline / roomAir / reoxygenated
        % and in the full-set analysis. Ids are then SPARSE -- gaps are cells with no
        % member in this subset -- and empty entries are skipped below.
        sid = C.curated.srcCellId(:);
        nId = max(sid(~isnan(sid)));
        cells = cell(nId,1);
        for i = 1:nId, cells{i} = find(sid == i); end
        src = 'curated + INHERITED source cell ids';
    else
...
% Stable content-derived key, identical to cell_link_260727's cell_key: the anchor
% member's <recording>#<roi>. cell_id is positional and renumbers whenever the ROI
% subset changes, so the key is what identifies a cell across analyses.
fovBase = strings(numel(fov_folder),1);
for f = 1:numel(fov_folder)
    q = char(fov_folder(f));
    while ~isempty(q) && (q(end)=='/'||q(end)=='\'), q(end)=[]; end
    [~,aa,bb] = fileparts(q);  fovBase(f) = string([aa bb]);
end
cellKey = strings(numel(cells),1);
for cc = 1:numel(cells)
    mm = cells{cc};
    if isempty(mm), continue; end        % sparse inherited ids leave gaps
    [~,ordk] = sortrows([fovBase(roi.fov(mm)), string(roi.roi(mm))]);
    aI = mm(ordk(1));
    cellKey(cc) = fovBase(roi.fov(aI)) + "#" + string(roi.roi(aI));
end
sel = find(sizes >= minCellSize & sizes > 0);
...
for si = 1:numel(sel)
    ...
    title(axL, sprintf('cell %d  (%dx)\nkey %s\nmedoid: %s #%d', ci, N, cellKey(ci), fovLabel(fov_name(f)), roi.roi(md)), ...
