...
% Experiment folder / genotype come from the shared config (coh_cfg_260727.m) so the
...
PP = init_coll();
labels = {};
% Explicit join key, saved alongside labels (260727). The label string is
% 'group/date/recName/roiIdx'; parsing it back out is fragile, and the cell-matching
% link needs (recording, ROI index) exactly. Keep them as their own arrays.
recNames = strings(0,1);
roiIdx   = zeros(0,1);
recPaths = strings(0,1);
confC  = NaN;
K_tap  = 2*TW - 1;
...
for kk = 1:numel(allMat)
    ...
    try
        ...
        for rid = 1:nROI
            ...
            recNames(end+1,1) = string(recName); %#ok<SAGROW>
            roiIdx(end+1,1)   = rid;             %#ok<SAGROW>
            recPaths(end+1,1) = string(folderPath); %#ok<SAGROW>
        end
...
if doSave
    ...
    for ii = sig_idx(:)'
        % 260727: the original split the label on '#', a character the label never
        % contains, so the fov and roi columns both came out as the whole label.
        % The recording name and ROI index are now tracked explicitly.
        fprintf(fid,'%s,%s,%.6f,%.6f,%.3f,"%s",%d\n', ...
                groups{PP.colorIdx(ii)}, labels{ii}, PP.r(ii), PP.th(ii), ...
                rad2deg(PP.th(ii)), recNames(ii), roiIdx(ii));
    end
    fclose(fid);
...
if doSave
    ...
    save(fullfile(outDir,'coherence_polar_data.mat'), ...
         ...
         'recNames','roiIdx','recPaths','ca_lag_sec');
