...
cohData  = cfg.cohData;
outDir   = fullfile(cfg.outRoot, 'dFF_heatmaps');

doSave = true;      % 260727: was false, so this script rendered but never wrote anything
...
%% ---- save ----
if doSave
    % 260727: FIGS 1-3 are commented out above, but this list still named f1..f3,
    % so the save block errored the first time doSave was ever true. Build the list
    % from whichever figures actually got created.
    want = {'f1','dFF_heatmap_time'; 'f2','dFF_heatmap_phase'; ...
            'f3','spike_hist_time';  'f4','spike_hist_phase'; ...
            'f5','cycle_dFF_sorted_time'; 'f6','cycle_dFF_sorted_phase'};
    out_pairs = cell(0,2);
    for k = 1:size(want,1)
        if exist(want{k,1},'var')
            h = eval(want{k,1});
            if ~isempty(h) && ishandle(h), out_pairs(end+1,:) = {h, want{k,2}}; end %#ok<AGROW>
        end
    end
    fprintf('Saving %d of %d figures (the rest are commented out above).\n', ...
            size(out_pairs,1), size(want,1));
    ...
    fprintf('Saved %d figures + .mat to %s\n', size(out_pairs,1), outDir);
