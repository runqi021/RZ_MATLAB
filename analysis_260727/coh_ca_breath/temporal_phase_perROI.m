% temporal_phase_perROI.m
% -----------------------------------------------------------------------
%  PER-ROI batch of temporal-phase figures. Renders ONE 12-panel temporal-phase
%  figure per selected ROI, across all ROIs that entered the coherence-polar
%  analysis. Which ROIs are rendered is controlled by `select`:
%     'sig'    : significant ROIs only (PP.r >= confC)
%     'active' : ROIs with more than minEvents spike events (nnz(spike_train)>minEvents)
%     'all'    : every ROI in the coherence list
%  (Merges the old temporal_phase_activeROI + temporal_phase_sigROI.)
%
%  ROI list source: coh_cfg_260727 -> coherence_polar_data.mat
%        labels{i} = 'group/date/recName/roi'  (every ROI used in coherence),
%        PP.r(i)   = coherence magnitude for that ROI.
%  Each label is resolved to a recording folder by recursive search for recName.
%
%  Per ROI: full analysis + 12-panel figure saved as:
%     <ROIfolder>\temporal_phase_svd_ROI##.png/.pdf
%     <rootPath>\temporal_phase_perROI_<select>\<select>##_group_date_roi##.png/.pdf
%
...
select   = 'active';        % which ROIs to render: 'sig' | 'active' | 'all'
outDir   = fullfile(cfg.outRoot, sprintf('temporal_phase_perROI_%s', select));
...
fprintf('%d ROIs in coherence list | select=%s (minEvents=%d, confC=%.4f)\n', ...
        numel(S.labels), select, minEvents, S.confC);
caCache = containers.Map('KeyType','char','ValueType','any');   % per-folder roi_spikes

%% ===================== LOOP =====================
nSel = 0; nSkip = 0; nFail = 0;
for k = 1:numel(S.labels)
    ...
    % ---- selection gate (sig | active | all) ----
    switch lower(select)
        case 'sig'
            if k > numel(S.PP.r) || S.PP.r(k) < S.confC, nSkip = nSkip + 1; continue; end
            tagStr = sprintf('r=%.3f', S.PP.r(k));
        case 'active'
            ...
            tagStr = sprintf('%d events', nEvents);
        otherwise   % 'all'
            tagStr = 'all';
    end
    ...
    nSel = nSel + 1;
    fprintf('\n[%d/%d] %s#%d %s | ROI %d (%s)\n   %s\n', ...
            k, numel(S.labels), select, nSel, lab, roi, tagStr, folderPath);
    try
        ...
        if doSave
            ...
            cbase = fullfile(outDir, sprintf('%s%02d_%s_%s_roi%02d', select, nSel, group, recDate, roi));
...
fprintf('\nDone. select=%s: %d figures rendered, %d skipped, %d failed. Batch dir: %s\n', ...
        select, nSel, nSkip, nFail, outDir);
