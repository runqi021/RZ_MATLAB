% temporal_phase_hist_population.m
% -----------------------------------------------------------------------
%  POPULATION spike-phase distributions, split into FOUR groups by the two
%  binary factors (IO or not) x (significant or not):
%       sig         : non-IO ROIs, r >= confC
%       nonsig      : non-IO ROIs, r <  confC
%       IO_sig      : IO ROIs,     r >= confC
%       IO_nonsig   : IO ROIs,     r <  confC
%  Empty groups are skipped automatically -- a single-genotype experiment (no
%  IO recordings) simply shows sig + nonsig.
%
%  (Consolidates the old temporal_phase_population + temporal_phase_population_groups.)
%
%  Each group is paginated: entity 1 = that group's POPULATION mean, then up to
%  perFigCells cell panels. Every entity = THREE panels:
%     [ time hist | phase hist (square) | polar ]
%   - time hist  : spikes (left axis, spk/cycle %) + breath-PEAK distribution
%                  (right axis), 0 = inspiration ONSET, tiled to 1.5 median period
%   - phase hist : spike phase 0..2pi (square)
%   - polar      : phase distribution line + black coherence (r,phase) dot from PP
%                  (no dot on the population panel)
%  RED = inspiration onset, SKY = breath-peak distribution.
%
%  Source: coh_cfg_260727 -> coherence_polar_data.mat (PP.colorIdx names the group,
%  PP.r/PP.th = coherence magnitude/preferred phase). Dependencies: detect_session_fps.m
% -----------------------------------------------------------------------
...
cohData  = cfg.cohData;
outDir   = fullfile(cfg.outRoot, 'temporal_phase_population');

perFigCells = 20;                   % cell panels per figure (large groups paginate)
...
% Rmax (ylim/RLim, coherence-r scaling) auto-snaps per entity to 20/40/60/80/100
...
%% ===================== FOUR GROUPS: (IO?) x (sig?) =====================
isIO  = (S.PP.colorIdx(:) == ioGroup);
isSig = (S.PP.r(:) >= S.confC);
grp = struct('name', {'sig','nonsig','IO_sig','IO_nonsig'}, ...
...
for s = 1:numel(grp)
    ...
    sel     = grp(s).sel;
    fprintf('\n=== %s : %d ROIs ===\n', setName, numel(sel));
    if isempty(sel), fprintf('  (skipped -- empty group)\n'); continue; end
    ...
    nC = size(M,1);
    if nC==0, warning('no cells collected for %s', setName); continue; end
    meanC = mean(M,1);
    % common time axis = group median breath period
    ...
    MP = zeros(nC, nTimeBins);   % breath PEAK times since onset (% of cycles)
    for i = 1:nC
