...
repoRoot = fileparts(fileparts(scriptDir)); addpath(repoRoot); addpath(scriptDir);
addpath(fullfile(repoRoot, '2p_breathing_coherence'));
addpath(genpath(fullfile(repoRoot, 'chronux_2_12')));

%% ===================== USER-EDITABLE =====================
% Single-recording inspector: pick ONE recording under the experiment root and
% one ROI. Experiment root comes from the shared config (coh_cfg_260727.m).
cfg        = coh_cfg_260727();
recName    = 'ro3_4x_x700y-1233_z-10_24.5lp_6000f_00001';  % <-- a recording folder under cfg.rootPath
folderPath = fullfile(cfg.rootPath, recName);
roi        = 24;            % ROI index (== dFF column == maskL label)
