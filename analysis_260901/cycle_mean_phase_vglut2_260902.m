% cycle_mean_phase_vglut2_260902.m
% -----------------------------------------------------------------------
%  Mean +/- SD of the per-cycle dF/F, on the same phase axis and from the same
%  cycles as the heatmaps, for the three Vglut2 significant groups:
%  vagal-intact FAST (0224), vagal-intact rest, and vagotomised.
%
...
% {display name, dates, mode, colour, linestyle}
GRP = { 'Vglut2 intact FAST (0224)', {'0224'},        'include', [0.45 0.80 0.35], '-'
        'Vglut2 intact rest',        {'0224','0824'}, 'exclude', [0.05 0.45 0.15], '-'
        'Vglut2 vagotomised',        {'0824'},        'include', [0.10 0.65 0.20], '--' };
GENOTYPE  = 'Vglut2';
nCycShow  = 3;
...
for gi = 1:nG
    ...
    for k = 1:numel(d)
        ...
        if ~(isKey(sigMap,stem) && sigMap(stem)), continue; end   % SIGNIFICANT only
    ...
    fprintf('%-28s %2d cells | onset %4d cycles | peak %4d cycles\n', ...
            GRP{gi,1}, NCELL(gi), NN(1,gi), NN(2,gi));
end

%% ===================== FIGURE =====================
fig = figure('Color','w','Units','centimeters','Position',[1 1 24 24], ...
             'Name','Vglut2 sig: mean per-cycle dF/F on phase');
...
x0 = [0.095 0.545];
y0 = [0.665 0.375 0.085];          % one row per group, top to bottom
for gi = 1:nG
    for q = 1:2
        ax = axes(fig,'Position',[x0(q), y0(gi), 0.375, 0.235]); %#ok<LAXES>
        ...
        fill(ax, [ph(ok) fliplr(ph(ok))], [m(ok)+sd(ok) fliplr(m(ok)-sd(ok))], ...
             GRP{gi,4}, 'FaceAlpha',0.20, 'EdgeColor','none');
        plot(ax, ph, m, GRP{gi,5}, 'Color',GRP{gi,4}, 'LineWidth',2.0);
...
annotation(fig,'textbox',[0.02 0.955 0.96 0.035],'EdgeColor','none', ...
    ...
    'String','Vglut2 SIGNIFICANT cells  |  mean per-cycle dF/F \pm 1 SD across cycles');
annotation(fig,'textbox',[0.02 0.925 0.96 0.030],'EdgeColor','none', ...
    'HorizontalAlignment','center','VerticalAlignment','middle','Interpreter','tex', ...
    'FontSize',8.5,'String',sprintf(['one group per panel, shared y scale  |  %d cycles, each ' ...
    'normalised by its own duration  |  red = cycle boundary'], nCycShow));
annotation(fig,'textbox',[0.02 0.898 0.96 0.030],'EdgeColor','none', ...
    'HorizontalAlignment','center','VerticalAlignment','middle','Interpreter','tex', ...
    'FontSize',8.5,'String',['grey = that group''s mean breath on the right axis, timing only ' ...
    '(SVD PC1, arbitrary units and sign)']);

stem = fullfile(outDir,'Vglut2_sig_cycle_mean_phase');
