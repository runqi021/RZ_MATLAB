% cycle_mean_phase_sert_260902.m
% -----------------------------------------------------------------------
%  Mean +/- SD of the per-cycle dF/F, on the same phase axis and from the same
%  cycles as the heatmaps, for two Sert groups:
%      vagal-intact (0721), SIGNIFICANT cells only
%      vagotomised (0828 + 0829), ALL ACTIVE cells
%
%  THE TWO GROUPS ARE SELECTED DIFFERENTLY ON PURPOSE, and that asymmetry is the
%  point of the figure, so it must not be read as a like-for-like comparison. It
%  was chosen because the vagotomised group has few locked cells; measured
%  2026-09-02, it has 8 of 60 (13%%), against 13 of 37 (35%%) intact -- fewer, but
%  NOT none. GRP's class column is per group, so setting the vagotomised row to
%  'sig' gives the matched comparison on those 8 cells instead.
%
...
GRP = { 'Sert intact, SIGNIFICANT',    {'0828','0829'}, 'exclude', 'sig', [0.90 0.45 0.10], '-'
        'Sert vagotomised, ALL active',{'0828','0829'}, 'include', 'all', [0.90 0.45 0.10], '--' };
GENOTYPE  = 'Sert';
nCycShow  = 3;
...
fig = figure('Color','w','Units','centimeters','Position',[1 1 24 17], ...
             'Name','Sert: mean per-cycle dF/F on phase');
...
annotation(fig,'textbox',[0.02 0.955 0.96 0.035],'EdgeColor','none', ...
    ...
    'String','Sert  |  mean per-cycle dF/F \pm 1 SD across cycles');
annotation(fig,'textbox',[0.02 0.925 0.96 0.030],'EdgeColor','none', ...
    'HorizontalAlignment','center','VerticalAlignment','middle','Interpreter','tex', ...
    'FontSize',8.5,'String',sprintf(['TOP = intact, SIGNIFICANT cells only.  BOTTOM = ' ...
    'vagotomised, ALL active cells -- the two rows are selected differently'], nCycShow));
...
stem = fullfile(outDir,'Sert_cycle_mean_phase');
