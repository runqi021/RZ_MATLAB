...
%% ============================== USER PARAMS ==============================
fovPath       = 'D:\Ventral_surface_summary\Vgat\0730\deep\roi1_3x_x1350y850_z265_3000f_31lp_00001';
roiIdxs       = [29];      % Ca ROIs to display; any count (1, 2, 3, ...).
                           % Tile stack = 1 breath tile + numel(roiIdxs) Ca tiles.

t_start       = 35;             % clip start (s)
t_end         = 65;            % clip end (s)
...
BEH_PRC_LO          = 1;       % lower percentile after crop
BEH_PRC_HI          = 99.5;    % upper percentile after crop
BEH_GAMMA           = 0.85;    % <1 brightens midtones; 1 = linear
...
TRACE_LABEL_RIGHT_DX    = 18;      % right edge of "Chest motion"/"ΔF/F" label is this px left of y-axis
...
% Y-limits.
% Auto by default. Overrides are keyed by ROI INDEX, not row position, so
% they stay attached to the right cell when you change roiIdxs.
% The chosen limits are printed on every run — copy a line into YLIM_BY_ROI
% and tweak it when the auto choice isn't what you want.
...
YLIM_BY_ROI = [             % [roiIdx  lo  hi];  ROIs not listed stay auto
    4   -0.2   0.4
];
...
nCa    = numel(roiIdxs);   % number of Ca ROI tiles (was hardwired to 3)
...
videoLabels = [{'Breath'}, ...
    arrayfun(@(r) sprintf('ROI %d',r), roiIdxs(:).', 'UniformOutput', false)];
...
% Manual overrides: breath row, then per-ROI keyed by index.
...
for k = 1:nCa
    if isempty(YLIM_BY_ROI); break; end
    hit = find(YLIM_BY_ROI(:,1) == roiIdxs(k), 1);
    if ~isempty(hit) && YLIM_BY_ROI(hit,3) > YLIM_BY_ROI(hit,2)
        trYLim(k+1,:) = YLIM_BY_ROI(hit,2:3);
        isManual(k)   = true;
    end
end
...
for k = 1:nCa
    ...
    fprintf('  ROI %-5d [%8.3f %8.3f]  %s\n', ...
            roiIdxs(k), trYLim(k+1,1), trYLim(k+1,2), src);
end

left_W = LEFT_LABEL_W + TILE_SZ;

tileStack_H = nTiles*TILE_SZ + (nTiles-1)*TILE_GAP;
left_H      = tileStack_H + X_AXIS_H;
...
if isempty(outFile)
    roiTag  = strjoin(arrayfun(@(r) sprintf('%d',r), roiIdxs(:).', ...
                              'UniformOutput', false), '-');
    outFile = fullfile(fovPath, sprintf('repr_ROI%s_t%d-%ds.mp4', ...
        roiTag, round(t_start), round(t_end)));
end
