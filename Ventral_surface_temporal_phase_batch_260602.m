% Ventral_surface_temporal_phase_batch_260602.m
% -----------------------------------------------------------------------
%  BATCH wrapper for Ventral_surface_temporal_phase_260601.m.
%  Renders ONE temporal-phase figure per SIGNIFICANT ROI (significant =
%  PP.r >= confC from the done coherence-polar analysis), for all 11
%  significant ROIs across the Ventral_surface_summary dataset.
%
%  Significant list source:
%     <rootPath>\coherence_polar_260528\coherence_polar_data.mat
%        PP.r, confC, labels{i} = 'group/date/recName/roi'
...
%  script) saved as:
%     <ROIfolder>\temporal_phase_ROI##.png/.pdf
%     <rootPath>\temporal_phase_batch_260602\sig##_group_date_roi##.png/.pdf
%
...
P.minSpikes    = 2;           P.ca_lag_sec   = 0.015;
...
for k = 1:numel(sigIdx)
    ...
    fprintf('\n[%d/%d] %s | ROI %d\n   %s\n', k, numel(sigIdx), lab, roi, folderPath);
    try
        fig = make_temporal_phase_fig(folderPath, roi, recName, group, recDate, P);
        if doSave
            base = fullfile(folderPath, sprintf('temporal_phase_ROI%02d', roi));
            ...
            cbase = fullfile(outDir, sprintf('sig%02d_%s_%s_roi%02d', k, group, recDate, roi));
...
fprintf('\nDone. %d figures rendered, %d failed. Batch dir: %s\n', nOK, nFail, outDir);
...
%% ---- spike histograms around peak ----
have_hist = false; sp_on_h = find(spk_train > 0);
peri_t_spk = []; peri_t_onset = [];
if ~isempty(on)
    for k = 1:numel(on)
        p = on(k);
...
pctC    = 100 * cntW / max(nCycles, eps);
rmaxC   = max([pctC, 1]);
ctrs_tileC = [ctrsP, ctrsP+2*pi];
if ~isempty(spkW), muW = mod(angle(mean(exp(1i*spkW))),2*pi); else, muW = NaN; end

%% ---- coherence method: BIT-FOR-BIT replica of Ventral_surface_coherence_polar_260528 ----
...
if have_coh && ~isempty(ip)
    ...
    if numel(peakC) >= 2 && numel(footC) >= 2
        ...
        [~, Cmag, cphi, ~,~,~, fC, confC, phistd, Cerr] = coherencyc(refC, stc-mean(stc), pcoh);
        ...
        th_coh = wrapToPi(th_coh - 2*pi*f_pk_c*P.ca_lag_sec);   % GCaMP comp (sig ROIs)
    end
end
...
% peri-peak time hist
if have_hist
    binT_sec = 0.033; edgesT = -win/fps : binT_sec : win/fps;
    ...
    cnt_sp = histcounts(peri_t_spk, edgesT); cnt_on = histcounts(peri_t_onset, edgesT);
    pct_sp = 100*cnt_sp / max(sum(cnt_sp),1); pct_on = 100*cnt_on / max(sum(cnt_on),1);
    yyaxis(ax_h1,'left');  bar(ax_h1, ctrsT, pct_sp, 1, 'FaceColor','k','FaceAlpha',0.85,'EdgeColor','none');
    ylabel(ax_h1,'spikes (%)'); set(ax_h1,'YColor','k');
    yyaxis(ax_h1,'right'); bar(ax_h1, ctrsT, pct_on, 1, 'FaceColor',onsetCol,'FaceAlpha',0.55,'EdgeColor','none');
    ylabel(ax_h1,'insp-onsets (%)'); set(ax_h1,'YColor',onsetCol);
    xline(ax_h1, 0, 'Color', peakCol, 'LineWidth', 1); xlim(ax_h1, [edgesT(1) edgesT(end)]);
    xlabel(ax_h1,'time from breath peak (s)');
    title(ax_h1, sprintf('#spikes, n=%d, #onsets=%d', numel(peri_t_spk), numel(peri_t_onset)),'Interpreter','none');
...
% linear 0..4pi (duplicated), spikes/cycle %
lax2 = nexttile(tl,tile_phist); hold(lax2,'on');
bar(lax2, ctrs_tileC, [pctC pctC], 1, 'FaceColor',[0.30 0.30 0.30],'FaceAlpha',0.75,'EdgeColor','none');
xlim(lax2,[0 4*pi]); ylim(lax2,[0 rmaxC*1.05]);
set(lax2,'XTick',[0 pi 2*pi 3*pi 4*pi],'XTickLabel',{'0','\pi','2\pi','3\pi','4\pi'});
