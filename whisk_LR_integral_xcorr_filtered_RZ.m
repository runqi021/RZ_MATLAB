...
%
% Population cross-correlation using fixed filtering standard:
%
% Whisker:
...
for ai = 1:numel(ad)
    ...
    for kk = 1:numel(rr)
        ...
        cc_sum = lagged_corr_fast(int_SUM, brw_bp1_20, mask, nLag, MIN_SAMPLES);
        cc_lr  = lagged_corr_fast(int_LR,  brw_bp1_20, mask, nLag, MIN_SAMPLES);
        acc_br = lagged_corr_fast(brw_bp1_20, brw_bp1_20, mask, nLag, MIN_SAMPLES);

        % Figure 2: angle auto/cross
        acc_L = lagged_corr_fast(xL, xL, mask, nLag, MIN_SAMPLES);
        acc_R = lagged_corr_fast(xR, xR, mask, nLag, MIN_SAMPLES);
        cc_LR = lagged_corr_fast(xL, xR, mask, nLag, MIN_SAMPLES);
        % cc_LR = corr[L(t+lag), R(t)]
        % lag > 0 means R leads L

        % Figure 3: integrals vs angles
        cc_sumL = lagged_corr_fast(int_SUM, xL, mask, nLag, MIN_SAMPLES);
        cc_sumR = lagged_corr_fast(int_SUM, xR, mask, nLag, MIN_SAMPLES);
        cc_lrL  = lagged_corr_fast(int_LR,  xL, mask, nLag, MIN_SAMPLES);
        cc_lrR  = lagged_corr_fast(int_LR,  xR, mask, nLag, MIN_SAMPLES);
        ...
        if DO_SHUFFLE
            ...
            if maxShift > minShift
                ...
                for si = 1:N_SHUFF
                    ...
                    cc_sum_sh(:,si) = lagged_corr_fast(int_SUM, br_sh, mask, nLag, MIN_SAMPLES);
                    cc_lr_sh(:,si)  = lagged_corr_fast(int_LR,  br_sh, mask, nLag, MIN_SAMPLES);
                    cc_LR_sh(:,si)  = lagged_corr_fast(xL, xR_sh, mask, nLag, MIN_SAMPLES);
