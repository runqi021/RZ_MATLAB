...
curUm = 0;  e1 = NaN;  e2 = NaN;  uPrev = NaN;  fnoPrev = -1;  lastNo = -1;  nSvc = 0;  nSkip = 0;
nEmpty = 0;
...
while ~strcmp(hSI.acqState,'idle')
    ...
    if inClosed && isfinite(dzNow)
        ...
        % EXTRAPOLATE THE DISTURBANCE, NOT THE RESIDUAL.
        % dzNow is the error that REMAINS after the command already applied, so
        % (e[k]-e[k-1]) is the residual's velocity -- and when the loop tracks
        % well that tends to ZERO, because the disturbance's motion and the
        % correction's motion cancel in it. Extrapolating it predicts nothing,
        % which is why gExtrap = 1 measurably did nothing: the 2026-09-13 sweep
        % fits a PURE DELAY of 1.46 frames to 1.0%% RMS, i.e. the old term was
        % netting to zero rather than costing.
        %
        % uStar is the command that would cancel the disturbance AS MEASURED --
        % remaining error plus what the mirror already holds. Its difference IS
        % the disturbance velocity, and leading by it is what buys back delay.
        % gExtrap is now "frames to lead by": 0 = off, 1.0 = theory (estimate
        % centroid to command centroid), ~1.5 = the measured effective delay.
        %
        % At gExtrap = 0 this is ALGEBRAICALLY IDENTICAL to the old form:
        %   want = corr + g*((corr - dz) - corr) = corr - g*dz.
        % NORMALISE THE VELOCITY BY FRAMES ACTUALLY ELAPSED.
        % The loop counts missed frames (nMissed / nSkip) but the control law
        % used to ignore them: after a drop, (uStar - uPrev) spans two or more
        % frame periods while gx multiplied it as though one had passed, so the
        % lead was 2x or more too large exactly when data was already sparse.
        % That injects a kick indistinguishable from a bad estimate, and is a
        % good candidate for why extrapolation looked useless on 2026-09-13.
        uStar = curUm - dzNow;
        dn    = max(1, fno - fnoPrev);          % frames since the last estimate
        if isfinite(uPrev)
            pred = uStar + EXTRAP_GAIN*(uStar - uPrev)/dn;
        else
            pred = uStar;
        end
        uPrev = uStar;  fnoPrev = fno;
        want  = curUm + LOOP_GAIN*(pred - curUm);
    ...
    elseif ~inClosed && curUm ~= 0
        curUm = 0;  e1 = NaN;  e2 = NaN;  uPrev = NaN;  fnoPrev = -1;
