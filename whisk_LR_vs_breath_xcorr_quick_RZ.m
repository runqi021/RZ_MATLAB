% whisk_insp_triggered_LR_BP_RZ.m
%
% Trigger-average BP whisker angle around inspiration events.
%
% Whisker:
...
%
% Event:
%   inspiration onset/event = findpeaks on inverted BP breath
%
% Output:
%   two plots:
...
PRE_S  = 0.30;        % s before inspiration event
POST_S = 0.30;        % s after inspiration event

MIN_INSP_DIST_S = 0.08;
PROM_MAD = 1.0;

MIN_MASK_FRAC = 0.80; % require this fraction of trigger window inside whisk epoch
...
for e = 1:numel(sess)
    ...
    try
        ...
        if ~isfile(Pn.breath)
            warning('LP-sub breath file not found %s n%d', animal, kRun);
            continue;
        end
...
sgtitle(sprintf('Triggered average: whisk BP %g-%g Hz, breath LP-sub inverted BP %g-%g Hz', ...
