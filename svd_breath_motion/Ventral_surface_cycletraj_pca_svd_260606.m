% Ventral_surface_cycletraj_pca_svd_260606.m
% -----------------------------------------------------------------------
%  TEMPORAL-TRACE trajectory via PCA (= SSA on a delay/Hankel embedding).
%  NOT a neuron state-space -- this embeds the 1-D temporal TRACE itself so
%  the breath cycle appears as an attractor LOOP.
%
...
%  The trace is z-scored, delay-embedded into a Hankel matrix (window = one median
%  breath cycle), and PCA gives the top-3 trajectory (one point/frame); the loop is
%  colored by piecewise breath phase (onset=0 red, peak=pi sky).
%
%  Parallel Isomap version: Ventral_surface_cycletraj_isomap_svd_260606.m
...
outDir   = fullfile(rootPath, 'traj_pca_svd_260606');
...
for r = 1:size(sigList,1)
    ...
    try
        ...
        phi  = F.phi(1:T);
        lag  = round(ca_lag_sec * F.fps);                 % calcium lead (frames)
        ...
        % ---- per-ROI trajectory = PCA over its cycles (embed the phase bins) ----
        [Yc, ec] = pca_traj(cycMat);
        fig = figure('Color','w','Name',sprintf('%s ROI%d traj PCA',recName,roi), ...
                     'Units','centimeters','Position',[3 3 17 14]);
        ax = axes('Parent',fig);
        draw_loop(ax, Yc, ec, phiBin(:), sprintf('%s ROI%d  |  PCA traj over %d breath-corr cycles  |  %s (+%dfr lead)', ...
                  recName, roi, popCycles, traceLbl, lag));
        if doSave
            base = fullfile(outDir, sprintf('traj_pca_%s_roi%02d', matlab.lang.makeValidName(recName), roi));
            ...
            save([base '.mat'], 'Yc','ec','cycMat','popCycles','roi','lag','recName');
        end
        ...
        nOK = nOK + 1;
        fprintf('[%d/%d] %s ROI%d  %d cycles  top3=%.0f%%\n', r, size(sigList,1), recName, roi, popCycles, sum(ec(1:3)));
    catch ME
...
if size(M,1) >= 3
    ...
    [Yp, ep] = pca_traj(M);                         % M = [Nsig x popCycles*nBins]; embed the obs
    figp = figure('Color','w','Name','population traj PCA','Units','centimeters','Position',[3 3 18 15]);
    axp = axes('Parent',figp);
    draw_loop(axp, Yp, ep, phiObs(:), sprintf('POPULATION PCA traj  |  %d sig ROIs x %d top breath-corr cycles', size(M,1), popCycles));
    if doSave
        base = fullfile(outDir,'traj_pca_POPULATION');
        ...
        save([base '.mat'], 'Yp','ep','M','labLong','phiObs','popCycles','nPhaseBins');
    end
    fprintf('POPULATION PCA: %d sig ROIs x %d cycles (%d obs), top3=%.0f%%\n', size(M,1), popCycles, size(M,2), sum(ep(1:3)));
end

%% ===================== LOCAL FUNCTIONS =====================
function [Y, explained] = pca_traj(M)
% Population trajectory: M = [neurons x bins]; embed the BINS (z-score per neuron).
...
function draw_loop(ax, Y, explained, phaseRows, ttl)
hold(ax,'on'); cmap = hsv(256);
plot3(ax, Y(:,1), Y(:,2), Y(:,3), '-', 'Color',[0.75 0.75 0.75], 'LineWidth',0.4);
cidx = max(1, ceil(phaseRows(:)/(2*pi)*256));
scatter3(ax, Y(:,1), Y(:,2), Y(:,3), 12, cmap(min(cidx,256),:), 'filled', 'MarkerFaceAlpha',0.6);
grid(ax,'on'); box(ax,'on'); view(ax,30,25); axis(ax,'tight');
xlabel(ax,sprintf('PC1 (%.0f%%)',explained(1)));
ylabel(ax,sprintf('PC2 (%.0f%%)',explained(2)));
zlabel(ax,sprintf('PC3 (%.0f%%)',explained(3)));
