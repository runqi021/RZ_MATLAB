% Ventral_surface_temporal_phase_sigROI_svd_260606.m
% -----------------------------------------------------------------------
%  SIGNIFICANT-ROI batch of Ventral_surface_temporal_phase.
%  Renders ONE temporal-phase figure per SIGNIFICANT ROI (PP.r >= confC from the
%  coherence-polar analysis) on the Ventral_surface_summary dataset.
%
%  ROI list source:
...
%     <rootPath>\temporal_phase_sigROI_svd_260606\sig##_group_date_roi##.png/.pdf
%  Active-ROI (>5 events) version: Ventral_surface_temporal_phase_activeROI_svd_260606.m
%
...
outDir   = fullfile(rootPath, 'temporal_phase_sigROI_svd_260606');
...
fprintf('%d significant ROIs (PP.r >= confC=%.4f) of %d total\n', numel(sigIdx), S.confC, numel(S.labels));

%% ===================== LOOP =====================
nSig = 0; nFail = 0;
for ii = 1:numel(sigIdx)
    k     = sigIdx(ii);
    lab   = S.labels{k};
    ...
    nSig = nSig + 1;
    fprintf('\n[%d/%d] sig#%d %s | ROI %d (r=%.3f)\n   %s\n', ...
            ii, numel(sigIdx), nSig, lab, roi, S.PP.r(k), folderPath);
    try
        ...
        if doSave
            ...
            cbase = fullfile(outDir, sprintf('sig%02d_%s_%s_roi%02d', nSig, group, recDate, roi));
            ...
            fprintf('   saved ROI%02d (folder + sig dir)\n', roi);
        end
...
fprintf('\nDone. %d significant figures rendered, %d failed. Sig dir: %s\n', nSig, nFail, outDir);
