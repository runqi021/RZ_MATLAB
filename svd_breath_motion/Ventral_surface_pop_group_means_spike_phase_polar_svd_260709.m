% Ventral_surface_pop_group_means_spike_phase_polar_svd_260709.m
% -----------------------------------------------------------------------
%  Just the three POPULATION-MEAN rows (entity 1 of each existing figure)
%  from the temporal_phase_batch_svd_260606 outputs, gathered into ONE
%  figure of 9 plots (3 rows x 3 panels):
%
%     row 1  ALL sig       : every ROI with PP.r >= confC        (= population_perROI_spike_phase_polar)
%     row 2  IO            : every inferior-olive ROI            (= IO_page01)
%     row 3  nonIO_nonsig  : non-IO ROIs with PP.r < confC       (= nonIO_nonsig_page01)
%
%  Each row = the per-cycle-normalized POPULATION MEAN across that group:
...
%   RED = inspiration onset (0), SKY = breath-peak distribution.
%
%  Same math/drawing as the two source scripts:
%   Ventral_surface_temporal_phase_population_svd_260606.m  (ALL sig)
%   Ventral_surface_temporal_phase_population_groups_svd_260606.m (IO / nonIO_nonsig)
%
...
%% ===================== THREE GROUPS (population means) =====================
groups = struct('name', {'ALL sig','IO','nonIO_nonsig'}, 'sel', {[],[],[]});
groups(1).sel = find(S.PP.r >= S.confC);
groups(2).sel = find(S.PP.colorIdx==ioGroup);
groups(3).sel = find(S.PP.colorIdx~=ioGroup & S.PP.r < S.confC);
...
for g = 1:numel(groups)
    ...
    fprintf('  %-14s : %d cells\n', groups(g).name, nC);
    G(g).name  = groups(g).name; G(g).nC = nC;
    G(g).meanC = meanC; G(g).meanT = meanT; G(g).meanP = meanP; G(g).ctrsT = ctrsT;
end

%% ===================== ONE FIGURE, 3 rows x [time | phase | polar] =====================
nCol = 3;
fig = figure('Color','w','Name','population-mean spike distributions (3 groups)', ...
             'Units','normalized','Position',[0.15 0.05 0.55 0.9]);
tl = tiledlayout(fig, 3, nCol, 'TileSpacing','compact','Padding','compact');
...
nC = size(M,1);
assert(nC>0, 'No ROI distributions collected for this group.');
meanC = mean(M,1);
