...
SNIFF_HZ   = 7;            % Hz, per-breath rate >  this = sniff  (RED)
SLOW_HZ    = 15;           % Hz, whisk frequency threshold for slow-whisking condition
...
function fig = render_overlay(XL, XR, BR, mb, ms, MAXPTS, T, WLIM, BLIM, TLIM, ttl)
    ...
    trigplot(subplot(3,3,8), T.lags_t, mL,  sL,  T.nT,  'insp-triggered L','xL', 'time from insp onset (s)', TLIM);
    [mR,sR]=msem(T.sXR, T.sXR2, T.nT);
    trigplot(subplot(3,3,9), T.lags_t, mR,  sR,  T.nT,  'insp-triggered R','xR', 'time from insp onset (s)', TLIM);
    sgtitle(ttl);
