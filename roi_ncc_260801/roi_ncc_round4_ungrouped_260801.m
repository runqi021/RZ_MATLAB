function roi_ncc_round4_ungrouped_260801()
...
%
% ---- 260727 changes vs the _260724 original (two bugs, both about losing work) ----
% 1) save_state now emits every remaining ungrouped ROI as its own 1x cell, which
%    is what the header always claimed and what rounds 2 and 3 already did. The
%    _260724 version wrote only grpOf>0, so a round-4 save silently dropped every
%    ungrouped observation from curatedCellMembers and from the CSV.
% 2) A SKIP in phase 2 is now remembered. It is recorded in a separate file
%    (roi_match_r4_reviewed.mat) rather than in grpOf, so an ROI you looked at and
%    chose to leave alone does not come back in phase 1 / phase 2 on the next run.
%    Kept in its own file so re-running rounds 1-3 cannot clobber it. Delete that
%    file to re-review everything from scratch.
%
% Runqi Zhang / 2026-07-27 (from roi_review4_ungrouped_260724.m)
close all;
...
maxAssignDist_um = 100;  % phase 2: only match an ungrouped ROI to cells within this
assignThr        = 0.4;  % phase 2: only suggest an assignment with medoid NCC > this
...
revR4 = load_reviewed(outDir, nRoiTot);   % ROIs already seen+skipped in a previous round 4
[pC, mC, Umat] = get_patches(outDir, roi, fov_folder, fov_pxum, params, nRoiTot);   % cache-first (fast)
fprintf('State: %d grouped (%d cells) | %d ungrouped (%d already reviewed) | %d tossed\n', ...
    nnz(grpOf>0), numel(unique(grpOf(grpOf>0))), nnz(grpOf==0), nnz(grpOf==0 & revR4), nnz(grpOf<0));
...
% Snapshot of never-grouped ROIs at entry, minus the ones you already decided to
% keep in a previous run (revR4) -- those are settled, don't ask again.
ung0 = find(grpOf == 0 & ~revR4);
...
while ~isempty(ung0)
    ...
    if btn == 1
        ...
        switch lower(char(btn))
            ...
            case 's', save_all(outDir, grpOf, homeOf, roi, fov_name, cellMembers, revR4); fprintf('saved\n');
            case 'q', break;
        end
    end
end
...
if goPhase2
    [rlist, clist, nccV, iouV, distV] = suggest_assign(grpOf, revR4, roi, Umat, mC, maxAssignDist_um);
    ...
    while qi <= numel(order)
        ...
        mem = find(grpOf==cid);
        if grpOf(r)~=0 || revR4(r) || isempty(mem) || any(roi.fov(mem)==roi.fov(r))
        ...
        if isempty(btn), break; end
        switch lower(char(btn))
            case 'y', grpOf(r) = cid;  revR4(r) = true;  qi = qi + 1;   % assign
            case {'n', ' ', char(29)}, revR4(r) = true;  qi = qi + 1;   % skip -> own 1x cell, and REMEMBERED
            case 'x', grpOf(r) = -1;   revR4(r) = true;  qi = qi + 1;   % toss
...
save_all(outDir, grpOf, homeOf, roi, fov_name, cellMembers, revR4);
fprintf('Round 4 done: %d grouped in %d cells | %d ungrouped kept as 1x cells | %d tossed.\n', ...
    nnz(grpOf>0), numel(unique(grpOf(grpOf>0))), nnz(grpOf==0), nnz(grpOf<0));
fprintf('  -> %s\n', fullfile(outDir,'roi_match_curated.mat'));
if ishandle(hFig), close(hFig); end
end

%% ---- assign / grid helpers ----
function [rlist, clist, nccV, iouV, distV] = suggest_assign(grpOf, revR4, roi, Umat, mC, maxDist)
% For each ungrouped, NOT-YET-REVIEWED ROI, its BEST-matching existing cell
% (medoid NCC), among cells within maxDist that DON'T already contain the ROI's
% FOV (one-per-FOV).
...
function save_all(outDir, grpOf, homeOf, roi, fov_name, cellMembers, revR4)
% One call so the curation and the round-4 review marks can never drift apart.
save_state(outDir, grpOf, homeOf, roi, fov_name, cellMembers);
save_reviewed(outDir, revR4);
end
...
%
% FIXED 260727: the _260724 version looped over grpOf>0 only, so every ungrouped
% ROI was dropped from curatedCellMembers and from the CSV the moment round 4
% saved -- the one round that is ABOUT ungrouped ROIs was the one that deleted
% them. This now matches rounds 2 and 3 exactly.
...
curated = struct('grpOf',grpOf,'homeOf',homeOf,'cellIds',posIds, ...
...
function rev = load_reviewed(outDir, nRoiTot)
% Round-4 "I looked at this and chose to leave it ungrouped" marks. Kept OUT of
% roi_match_curated.mat on purpose: rounds 1-3 rewrite that file wholesale and
% would silently drop an extra field.
rev = false(nRoiTot,1);
fp = fullfile(outDir,'roi_match_r4_reviewed.mat');
if ~isfile(fp), return; end
S = load(fp);
if isfield(S,'reviewed') && numel(S.reviewed)==nRoiTot
    rev = logical(S.reviewed(:));
else
    warning('roi_match_r4_reviewed.mat ROI count mismatch - ignoring it (everything re-reviewed).');
end
end

function save_reviewed(outDir, rev)
reviewed = logical(rev(:)); %#ok<NASGU>
save(fullfile(outDir,'roi_match_r4_reviewed.mat'),'reviewed');
end
