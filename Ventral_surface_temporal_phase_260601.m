% Ventral_surface_temporal_phase_260601.m
% -----------------------------------------------------------------------
...
pctC    = 100 * cntW / max(nCycles, eps);    % avg spikes/cycle per phase bin (%)
rmaxC   = max([pctC, 1]);
...
if have_coh
    ...
    th_coh = wrapToPi(th_coh - 2*pi*f_pk*ca_lag_sec);   % GCaMP-rise compensation
end
...
tile_peri  = 18;                        % row 5    : peri-peak time hist
...
% ===== tile_peri : peri-peak time hist (spikes + onsets) =====
if have_hist
    ...
    pct_sp = 100*cnt_sp / max(sum(cnt_sp),1);
    pct_on = 100*cnt_on / max(sum(cnt_on),1);
    yyaxis(ax_h1,'left');
    bar(ax_h1, ctrsT, pct_sp, 1, 'FaceColor', 'k', 'FaceAlpha',0.85, 'EdgeColor','none');
    ylabel(ax_h1,'spikes (%)'); set(ax_h1,'YColor','k');
    yyaxis(ax_h1,'right');
    bar(ax_h1, ctrsT, pct_on, 1, 'FaceColor', onsetCol, 'FaceAlpha',0.55, 'EdgeColor','none');
    ylabel(ax_h1,'insp-onsets (%)'); set(ax_h1,'YColor',onsetCol);
    xline(ax_h1, 0, 'Color', peakCol, 'LineWidth', 1);
...
% ===== tile_phist : LINEAR 0..4pi (single cycle duplicated), spikes/cycle % =====
