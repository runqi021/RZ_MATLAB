...
%    R4 Sorted peak SIG  Peak-trig avg NSIG  Sorted cycle SIG  Cycle-avg NSIG    KDE
%
%  Auto-detect mode:
...
TW              = 4;              % multitaper time-bandwidth (K = 2*TW-1 tapers)
...
ibi_xlim        = [0 2.5];         % x/y limits for Poincare and KDE
...
% Dual coherence collectors (waveform vs spike train)
all_th_k_wave  = [];   all_r_k_wave  = [];
all_rlo_wave   = [];   all_rhi_wave  = [];   all_dphi_wave = [];
all_th_k_spk   = [];   all_r_k_spk   = [];
all_rlo_spk    = [];   all_rhi_spk   = [];   all_dphi_spk  = [];
all_is_sig_wave = [];  all_is_sig_spk = [];
...
for kk = 1:nSess
    ...
    try
        ...
        for jj = 1:nSpkROI
            ...
            [~, C_w, phi_w, ~, ~, ~, f_coh, ~, phistd_w, Cerr_w] = ...
                coherencyc(breath_rs, sig_c(1:T_coh), params_coh);
            f_coh = f_coh(:); C_w = C_w(:); phi_w = phi_w(:);

            mask_band = f_coh >= f_coh_band(1) & f_coh <= f_coh_band(2);
            if ~any(mask_band), mask_band = true(size(f_coh)); end

            r_k_w    = mean(C_w(mask_band));
            th_k_w   = angle(mean(exp(1i * (-phi_w(mask_band)))));
            dphi_k_w = mean(phistd_w(mask_band));
            rlo_k_w  = max(0, mean(Cerr_w(1, mask_band)));
            rhi_k_w  = min(1, mean(Cerr_w(2, mask_band)));
            ...
            spk_train = double(spk_train) - mean(spk_train);

            [~, C_s, phi_s, ~, ~, ~, ~, ~, phistd_s, Cerr_s] = ...
                coherencyc(breath_rs, spk_train, params_coh);
            C_s = C_s(:); phi_s = phi_s(:);

            r_k_s    = mean(C_s(mask_band));
            th_k_s   = angle(mean(exp(1i * (-phi_s(mask_band)))));
            dphi_k_s = mean(phistd_s(mask_band));
            rlo_k_s  = max(0, mean(Cerr_s(1, mask_band)));
            rhi_k_s  = min(1, mean(Cerr_s(2, mask_band)));

            % --- Select which coherence drives is_sig ---
            if coherence_use_spike
                r_k = r_k_s;  th_k = th_k_s;  dphi_k = dphi_k_s;
                rlo_k = rlo_k_s;  rhi_k = rhi_k_s;
            else
                r_k = r_k_w;  th_k = th_k_w;  dphi_k = dphi_k_w;
                rlo_k = rlo_k_w;  rhi_k = rhi_k_w;
            end
            ...
            % Dual coherence accumulators (always both)
            all_th_k_wave  = [all_th_k_wave;  th_k_w];    %#ok<AGROW>
            all_r_k_wave   = [all_r_k_wave;   r_k_w];     %#ok<AGROW>
            all_rlo_wave   = [all_rlo_wave;   rlo_k_w];    %#ok<AGROW>
            all_rhi_wave   = [all_rhi_wave;   rhi_k_w];    %#ok<AGROW>
            all_dphi_wave  = [all_dphi_wave;  dphi_k_w];   %#ok<AGROW>
            all_is_sig_wave = [all_is_sig_wave; r_k_w > confC_val]; %#ok<AGROW>

            all_th_k_spk   = [all_th_k_spk;   th_k_s];    %#ok<AGROW>
            all_r_k_spk    = [all_r_k_spk;    r_k_s];     %#ok<AGROW>
            all_rlo_spk    = [all_rlo_spk;    rlo_k_s];    %#ok<AGROW>
            all_rhi_spk    = [all_rhi_spk;    rhi_k_s];    %#ok<AGROW>
            all_dphi_spk   = [all_dphi_spk;   dphi_k_s];   %#ok<AGROW>
            all_is_sig_spk = [all_is_sig_spk; r_k_s > confC_val]; %#ok<AGROW>
...
%% ---- (R4,C1): Sorted per peak hmap SIG (time) ----
...
%% ---- (R4,C2): Peak-trig avg hmap NSIG (time) ----
ax = axes(fig, 'Units','centimeters','Position',[col2_x row4_y sq_w sq_h]);
if nNonSigROIs > 0
    peth_sorted_nsig_mat = peth_mat_all(sort_ord_nsig, :);
    labels_sorted_nsig   = labels_all(sort_ord_nsig);
    imagesc(ax, t_w, 1:nNonSigROIs, peth_sorted_nsig_mat);
    set(ax, 'YDir','reverse');
    colormap(ax, flipud(gray(256)));
    ...
    if nNonSigROIs <= 40
        ...
        ts = max(1, round(nNonSigROIs/20));
        yt = 1:ts:nNonSigROIs;
        set(ax, 'YTick', yt, 'YTickLabel', labels_sorted_nsig(yt), 'FontSize', 5);
    end
    xlabel(ax, 'Time (s)'); ylabel(ax, 'ROI');
    title(ax, sprintf('Peak-trig avg n.s. (%d)', nNonSigROIs));
    cb = colorbar(ax); ylabel(cb, 'dF/F');
    ...
    text(ax, 0.5,0.5, 'No n.s. ROIs', 'HorizontalAlignment','center', 'Units','normalized');
    title(ax, 'Peak-trig avg n.s. (0)');
end
...
%% ---- (R4,C3): Sorted per cycle hmap SIG (phase) ----
...
%% ---- (R4,C4): Cycle-avg hmap NSIG (phase) ----
ax = axes(fig, 'Units','centimeters','Position',[col4_x row4_y sq_w sq_h]);
if nNonSigROIs > 0
    avg_nsig_dup = [avg_phase_nsig, avg_phase_nsig];
    imagesc(ax, phase_full, 1:nNonSigROIs, avg_nsig_dup);
    set(ax, 'YDir','reverse');
    colormap(ax, flipud(gray(256)));
    ...
    set(ax, 'XTick', [0 pi 2*pi 3*pi 4*pi], ...
        'XTickLabel', {'0','\pi','2\pi','3\pi','4\pi'});
    if nNonSigROIs <= 40
        set(ax, 'YTick', 1:nNonSigROIs, 'YTickLabel', avg_labels_phase_nsig, 'FontSize', 5);
    else
        ts = max(1, round(nNonSigROIs/20));
        yt = 1:ts:nNonSigROIs;
        set(ax, 'YTick', yt, 'YTickLabel', avg_labels_phase_nsig(yt), 'FontSize', 5);
    end
    xlabel(ax, 'Phase'); ylabel(ax, 'ROI');
    title(ax, sprintf('Cycle-avg n.s. (%d)', nNonSigROIs));
    cb = colorbar(ax); ylabel(cb, 'dF/F');
...
if numel(valid_psd_idx) >= 1
    ...
    for ii = 1:size(psd_mat, 1)
        plot(ax, f_common, 10*log10(psd_mat(ii,:)), '-', 'Color', [0.6 0.6 0.6 0.5], 'LineWidth', 0.5);
    end
    m_lin = mean(psd_mat, 1, 'omitnan');
    % s_lin = std(psd_mat, 0, 1, 'omitnan');
    % n_valid = sum(~isnan(psd_mat), 1);
    % sem_lin = s_lin ./ sqrt(n_valid);
...
%% ---- Save coherence data for master summary ----
coherence_data = struct();
coherence_data.all_th_k_wave  = all_th_k_wave;
coherence_data.all_r_k_wave   = all_r_k_wave;
coherence_data.all_rlo_wave   = all_rlo_wave;
coherence_data.all_rhi_wave   = all_rhi_wave;
coherence_data.all_dphi_wave  = all_dphi_wave;
coherence_data.all_is_sig_wave = all_is_sig_wave;
coherence_data.all_th_k_spk   = all_th_k_spk;
coherence_data.all_r_k_spk    = all_r_k_spk;
coherence_data.all_rlo_spk    = all_rlo_spk;
coherence_data.all_rhi_spk    = all_rhi_spk;
coherence_data.all_dphi_spk   = all_dphi_spk;
coherence_data.all_is_sig_spk = all_is_sig_spk;
coherence_data.all_roi_sess   = all_roi_sess;
coherence_data.all_roi_id     = all_roi_id;
coherence_data.confC_mean     = confC_mean;
coherence_data.labels_all     = labels_all;
coherence_data.inputPath      = inputPath;
save(fullfile(outDir_map, 'coherence_data.mat'), 'coherence_data', '-v7.3');
fprintf('Saved coherence_data.mat\n');

%% =====================================================================
%%  FIGURE: Coherence comparison — waveform vs spike train (two polar plots)
%% =====================================================================
coh_fig_w = 2*sq_w + hgap + margin_l + margin_r;
coh_fig_h = margin_b + sq_h + margin_t;

figCoh = figure('Color','w', ...
    'Name', 'Coherence: waveform vs spike', ...
    'Units','centimeters', 'Position', [1 1 coh_fig_w coh_fig_h], ...
    'PaperUnits','centimeters', 'PaperSize', [coh_fig_w coh_fig_h], ...
    'PaperPosition', [0 0 coh_fig_w coh_fig_h]);
set(figCoh, 'DefaultAxesFontSize', 7, 'DefaultTextFontSize', 7);

% --- Left: waveform coherence polar ---
pol_w_norm = [margin_l/coh_fig_w, margin_b/coh_fig_h, sq_w/coh_fig_w, sq_h/coh_fig_h];
ax_cw = polaraxes(figCoh, 'Position', pol_w_norm);
hold(ax_cw, 'on');
th_circ_coh = linspace(0, 2*pi, 360);
polarplot(ax_cw, th_circ_coh, repmat(confC_mean, 1, 360), 'k--', 'LineWidth', 1);
for rr = 1:nTotalROIs
    if all_is_sig_wave(rr)
        c_r = sessColor(all_roi_sess(rr), :);
    else
        c_r = [0 0 0];
    end
    polarplot(ax_cw, all_th_k_wave(rr), all_r_k_wave(rr), 'o', ...
        'MarkerFaceColor', c_r, 'MarkerEdgeColor', 'k', 'MarkerSize', 3);
end
ax_cw.RLim = [0, 1];
ax_cw.ThetaZeroLocation = 'top';
ax_cw.ThetaDir = 'clockwise';
ax_cw.RAxisLocation = 180;
ax_cw.FontSize = 7;
hold(ax_cw, 'off');
nSig_w = sum(all_is_sig_wave);
title(ax_cw, sprintf('Waveform (%d/%d sig)', nSig_w, nTotalROIs), 'Interpreter','none');

% --- Right: spike-train coherence polar ---
col2_coh_x = margin_l + sq_w + hgap;
pol_s_norm = [col2_coh_x/coh_fig_w, margin_b/coh_fig_h, sq_w/coh_fig_w, sq_h/coh_fig_h];
ax_cs = polaraxes(figCoh, 'Position', pol_s_norm);
hold(ax_cs, 'on');
polarplot(ax_cs, th_circ_coh, repmat(confC_mean, 1, 360), 'k--', 'LineWidth', 1);
for rr = 1:nTotalROIs
    if all_is_sig_spk(rr)
        c_r = sessColor(all_roi_sess(rr), :);
    else
        c_r = [0 0 0];
    end
    polarplot(ax_cs, all_th_k_spk(rr), all_r_k_spk(rr), 'o', ...
        'MarkerFaceColor', c_r, 'MarkerEdgeColor', 'k', 'MarkerSize', 3);
end
ax_cs.RLim = [0, 1];
ax_cs.ThetaZeroLocation = 'top';
ax_cs.ThetaDir = 'clockwise';
ax_cs.RAxisLocation = 180;
ax_cs.FontSize = 7;
hold(ax_cs, 'off');
nSig_s = sum(all_is_sig_spk);
title(ax_cs, sprintf('Spike train (%d/%d sig)', nSig_s, nTotalROIs), 'Interpreter','none');
...
if nPsdSess > 0
    ...
    for ii = 1:nPsdSess
        ...
        hold(ax2, 'on');
        fwhm_kk = all_f_fwhm{kk_};
        if ~isempty(fwhm_kk)
            patch(ax2, [fwhm_kk(1) fwhm_kk(2) fwhm_kk(2) fwhm_kk(1)], ...
                [yLo yLo yHi yHi], [1 0.7 0.7], 'EdgeColor','none');
        end
        ...
        hold(ax3, 'on');
        if ~isempty(fwhm_kk)
            patch(ax3, [fwhm_kk(1) fwhm_kk(2) fwhm_kk(2) fwhm_kk(1)], ...
                [0 0 all_cum_max*1.05 all_cum_max*1.05], [1 0.7 0.7], 'EdgeColor','none');
        end
