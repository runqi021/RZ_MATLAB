...
assert(~isempty(csv_hits), "No DLC CSV found (*snapshot_200, *best_20, or *best_10).");
...
for k = 1:numel(roiList)
    ...
    ax12.XLim   = [min(fmt(1), fbC(1)), fmax];
    grid(ax12,'on');
    ...
    xline(ax22, 0, 'k--', 'LineWidth',1.2);
    %xline(ax22, -pi, '--', 'Color',[0.5 0.5 0.5], 'LineWidth',0.8);
    %xline(ax22,  pi, '--', 'Color',[0.5 0.5 0.5], 'LineWidth',0.8);
    ax22.XLim   = [-2*pi, 2*pi];
    ax22.YLim(1) = 0;
    ax22.XTick  = [-2*pi -pi 0 pi 2*pi];
    ax22.XTickLabel = {'-2\pi', '-\pi', '0', '\pi', '2\pi'};
    ...
    % Square all cartesian axes via PlotBoxAspectRatio (works with layout engine)
    for axi = {ax11, ax21, ax31, ax12, ax22}
        pbaspect(axi{1}, [1 1 1]);
    end
    drawnow;

    % Coherence inset in bottom-left corner of PSD tile (ax12)
    % Use TightInset to find where the actual plot box sits inside Position
    ti  = ax12.TightInset;          % [left bottom right top] margins
    rp  = ax12.Position;
    bx  = rp(1) + ti(1);
    by  = rp(2) + ti(2);
    bw  = rp(3) - ti(1) - ti(3);
    bh  = rp(4) - ti(2) - ti(4);
    inW = 0.38 * bw;    inH = 0.38 * bh;
    inX = bx + 0.20 * bw;
    inY = by - 0.30 * bh;
