...
% Set all figure fonts to Arial
set(0, 'DefaultAxesFontName', 'Arial');
set(0, 'DefaultTextFontName', 'Arial');

folderPath = "D:\batch_dffQC_test_260325\260323_vgat_g8s\phys\processed\-500-990-z-30_8x_3000f_00001";
...
for kk = 1:N_roi
    plot(ax2b, t_img, Y(:,kk), 'Color', 'k', 'LineWidth', 1);
end
...
% Color segments GREEN if they contain a calcium spike for that ROI
...
for i = 1:N_roi
    ...
    % Check which segments contain a calcium spike for this ROI
    spk_idx_this = ca_spk_data(i).spike_idx;   % frame indices of spikes
    has_spike = false(nEv, 1);
    for e = 1:nEv
        seg_start = insp_ev(e) - win;
        seg_end   = insp_ev(e) + win;
        if any(spk_idx_this >= seg_start & spk_idx_this <= seg_end)
            has_spike(e) = true;
        end
    end
    ...
    % Plot gray trials (no spike)
    idx_gray = find(~has_spike);
    if ~isempty(idx_gray)
        plot(t_evt, seg(idx_gray,:)', 'Color', [0.8 0.8 0.8], 'LineWidth', 0.7);
    end
    % Plot green trials (has spike)
    idx_green = find(has_spike);
    if ~isempty(idx_green)
        plot(t_evt, seg(idx_green,:)', 'Color', [0.2 0.7 0.2], 'LineWidth', 0.7);
    end
    ...
    ylabel('\DeltaF/F');
    title(sprintf('ROI#%d (%d/%d spk)', i, sum(has_spike), nEv));
...
for k = 1:nSpkROI
    ...
    % --- LEFT: breath-triggered dFF overlay with green spike coloring ---
    seg_breath = dff_evt_seg_all{roi_id};   % nEv x (2*win+1)
    spk_idx_this = ca_spk_data(roi_id).spike_idx;
    has_spike = false(nEv, 1);
    for e = 1:nEv
        seg_start = insp_ev(e) - win;
        seg_end   = insp_ev(e) + win;
    ...
    idx_gray = find(~has_spike);
    if ~isempty(idx_gray)
        plot(t_evt, seg_breath(idx_gray,:)', 'Color', [0.8 0.8 0.8], 'LineWidth', 0.7);
    end
    idx_green = find(has_spike);
    if ~isempty(idx_green)
        plot(t_evt, seg_breath(idx_green,:)', 'Color', [0.2 0.7 0.2], 'LineWidth', 0.7);
    end
    plot(t_evt, dff_avg, 'Color', 'k', 'LineWidth', 2);
    ...
    xlabel('Time from inspiration onset (s)');
    ylabel('\DeltaF/F');
    title(sprintf('ROI#%02d  breath-trig dFF (%d/%d spk)', roi_id, sum(has_spike), nEv));
    axis square;
    hold off;
...
%% plot histogram for phase preference
...
off = 1.5*gap*(nSpkROI - (1:nSpkROI));   % same vertical offsets you used

tickCenter = off - 0.50*gap;         % where the raster sits (in the gap below each trace)
tickHalf   = 0.12*gap;               % half height of each tick

hold on
for k = 1:nSpkROI
    spk = find(ca_spk_train(:,k));   % spike indices (time bins)
    if isempty(spk), continue; end

    x = spk;                         % OR x = t(spk) if you have a time vector t
    xx = [x(:) x(:)]';               % 2 x N
    yy = [(tickCenter(k)-tickHalf)*ones(numel(spk),1), ...
          (tickCenter(k)+tickHalf)*ones(numel(spk),1)]';  % 2 x N

    plot(xx/fs, yy, 'k', 'LineWidth', 1);  % draws N little vertical lines
end
xlabel('Time (s)')
...
%% Phase histogram per spiking ROI
for k = 1:nSpkROI
    roi_id = roiSpk_id(k);
