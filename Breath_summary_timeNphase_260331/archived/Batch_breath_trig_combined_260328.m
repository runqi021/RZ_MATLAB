...
skipIdx        = [];          % session indices to skip (master mode only)
...
clip_lo        = 0.4;         % brightness clip percentile (low)
...
nPhaseBins     = 200;         % bins for phase-normalized breath cycles (summary)
hmap_win_sec   = 0.5;         % half-window for per-ROI heatmap (seconds); [] = auto (2.5x mean IBI)
...
%% 2 ── Add Chronux and helper package to path
...
helperParent = fullfile(fileparts(mfilename('fullpath')), '2p_breathing_coherence');
if isfolder(helperParent) && ~contains(path, '2p_breathing_coherence')
    addpath(helperParent);
    fprintf('Added helper package to path: %s\n', helperParent);
end
...
for kk_sess = 1:nSess
    ...
    try
        ...
        if ~isempty(hmap_win_sec)
            win_sec = hmap_win_sec;
        else
        ...
        for kk_roi = 1:nSpkROI
            ...
            polarplot(ax_42, [th_k th_k], [rlo rhi], '-', 'Color', col, 'LineWidth', 2);
            phi_arc = linspace(th_k - dphi, th_k + dphi, 40);
            polarplot(ax_42, phi_arc, r_k * ones(1,40), '-', 'Color', col, 'LineWidth', 1.5);
            ph = polarplot(ax_42, th_k, r_k, 'o', ...
