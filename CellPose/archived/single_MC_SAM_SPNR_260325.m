...
%   - Two-pass non-rigid (SPNR) MC -> cpSAM
...
%
% Non-rigid (SPNR) patch-based motion correction:
%   PatchSizeUm (default 128 um) is auto-converted to pixels from zoom.
%   The image is divided into overlapping patches, each corrected independently,
%   with smooth interpolation at patch boundaries to avoid edge artifacts.
%   UsFac controls subpixel registration (1=integer shifts, 4=quarter-pixel).
%   MaxDev limits how much adjacent patches can differ in shift (pixels).
%
% Multi-pass MC:
...
%
% Requires on path:
%   loadtiff, saveastiff, run_spnr_mc, cp_sam_extract_F_cli
% Optional:
%   helper.dFF_RZ
clear; clc; delete(gcp('nocreate'));

%% ============================== USER ==============================
filePath = "D:\batch_dffQC_test_260325\260322_sst_soma_g8s\phys\processed\maybe breathing\6x_-820-1070-z20_00001\6x_-820-1070-z20_00001.tif";   % <<< exact TIFF path
...
% -------- frame selection --------
KeepFrames = Inf;   % after deinterleave, keep this many frames (0 or Inf = keep all)
...
% -------- MC (non-rigid SPNR) --------
MaxShiftUm     = 10;      % max physical shift per patch (um) — auto-converted to pixels from zoom
PatchSizeUm    = 128;     % patch size (um) — auto-converted to pixels from zoom
MaxDev         = [5 5];   % max shift deviation between adjacent patches (pixels)
UsFac          = 1;       % subpixel upsampling (1=integer, 4=quarter-pixel)
PixelSizeBase  = 1.7778;  % um/px at 1x zoom (calibrated from 6x, 4.2x, 1.2x data)
% MC is always 2-pass: pass 1 = rigid (fast template), pass 2 = SPNR (non-rigid refinement)
...
%% ============================== RUN ==============================
out = run_single_MC_SAM_SPNR(filePath, pyExe, ...
    ...
    "MaxShiftUm",MaxShiftUm, "PixelSizeBase",PixelSizeBase, ...
    "PatchSizeUm",PatchSizeUm, "MaxDev",MaxDev, "UsFac",UsFac, ...
...
%% ======================================================================
function out = run_single_MC_SAM_SPNR(filePath, pyExe, varargin)
...
p.addParameter("PatchSizeUm", 128, @(x)isnumeric(x)&&isscalar(x)&&x>0);
p.addParameter("MaxDev", [5 5], @(x)isnumeric(x)&&numel(x)==2);
p.addParameter("UsFac", 1, @(x)isnumeric(x)&&isscalar(x)&&x>=1);
% MCpasses removed — always 2-pass (rigid then SPNR)
...
% --- auto max_shift and patch_size from zoom factor ---
meta = "";
...
if ~isempty(zoomFactor) && isfinite(zoomFactor) && zoomFactor > 0
    ...
    patch_size_px = max(round(pr.PatchSizeUm / pixel_size_um), 32);
    fprintf("[MC] zoom=%.2fx, pixel=%.4f um/px, MaxShift=%d um -> %d px, PatchSize=%d um -> %d px\n", ...
        zoomFactor, pixel_size_um, pr.MaxShiftUm, max_shift_px, pr.PatchSizeUm, patch_size_px);
else
    max_shift_px  = 20;
    patch_size_px = 128;
    fprintf(2, "[MC] WARNING: could not read scanZoomFactor from metadata. Using default max_shift=%d px, patch_size=%d px\n", max_shift_px, patch_size_px);
end
...
out_mc_mat   = outFolder + filesep + stem + "_SPNRinfo.mat";
out_mc_png   = outFolder + filesep + stem + "_SPNRshift.png";
out_mc_guess = outFolder + filesep + stem + "_preproc_SPNR.tif";

out_sam_guess = outFolder + filesep + stem + "_preproc_SPNR_cpSAM_output.mat";
...
% --- STEP 2: MC (skip if MC tif exists OR SPNRinfo exists + can infer tif) ---
mc_tif = "";
...
if mc_tif == ""
    mc_tif = newest_matching_file(outFolder, stem + "_preproc*SPNR*.tif");
...
if mc_tif ~= "" && isfile(mc_tif) && isfile(out_mc_mat)
    ...
    fprintf("[RUN ] MC step (pass 1: rigid, pass 2: non-rigid SPNR, max_shift=%d px, patch=%d px)\n", max_shift_px, patch_size_px);

    mc_input = out_preproc_tif;

    % --- Pass 1: RIGID (fast, builds clean template) ---
    fprintf("[MC pass 1/2] RIGID, max_shift=%d px\n", max_shift_px);
    mcOut_rigid = run_rigid_mc(mc_input, ...
        'MaxShift', max_shift_px, ...
        'InitBatch', pr.MCinitBatch, ...
        'BinWidth', pr.MCbinWidth, ...
        'TossFrames', pr.MCTossFrames);
    mc_input = string(mcOut_rigid.mc_path);

    % --- Pass 2: NON-RIGID SPNR (precise, uses template from rigid pass) ---
    pass2_max_shift = max(round(max_shift_px / 2), 1);
    fprintf("[MC pass 2/2] SPNR, max_shift=%d px, patch=%d px\n", pass2_max_shift, patch_size_px);
    mcOut = run_spnr_mc(mc_input, ...
        'MaxShift', pass2_max_shift, ...
        'PatchSize', [patch_size_px patch_size_px], ...
        'MaxDev', pr.MaxDev, ...
        'UsFac', pr.UsFac, ...
        'InitBatch', pr.MCinitBatch, ...
        'BinWidth', pr.MCbinWidth, ...
        'Template', mcOut_rigid.template);

    mc_tif = string(mcOut.mc_path);
    assert(mc_tif ~= "" && isfile(mc_tif), "Could not locate MC tif output.");

    % save MC results first (protect expensive computation)
    sx = []; sy = []; r = [];
    save(char(out_mc_mat), "mcOut","mcOut_rigid","mc_tif","sx","sy","r","max_shift_px","patch_size_px","-v7.3");
    fprintf("Saved MC info:\n%s\n", out_mc_mat);

    % shift plot (from final pass) — try-catch to not lose MC results
    try
        if isfield(mcOut,"shifts")
            ...
            title('|SPNR shift| per frame (pass 2)');
