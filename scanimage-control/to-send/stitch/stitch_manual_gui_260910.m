...
S.datasetRoot = "E:\260804_shiver_dbh\map";
S.refChID     = 3;
S.zUse_um     = [-300 0]; S.zUseProj    = [];
S.projMode    = 'avg';    S.workSpace   = "raw";

%%
S.clipLo      = 1;  S.clipHi      = 99.5;   S.gamma       = 0.8;

S.moveStep    = 5;  S.feather_um  = 30;
S.dsFast      = 4;  S.dsIdle      = 1;      S.idleSec     = 0.30;

repoRoot = fileparts(fileparts(mfilename('fullpath'))); addpath(repoRoot);
here = fileparts(mfilename('fullpath')); addpath(here);

BG   = [0.13 0.13 0.13];    FG   = [0.92 0.92 0.92];    DIM  = [0.55 0.55 0.55];
...
cb_load();
    ...
    function autoBC(write)
        ...
        for t = 1:S.nTiles
            P = proj(t);
            v = [v; single(reshape(P(1:6:end,1:6:end), [], 1))];
        end
    ...
    function cb_save()
        ...
        workSpace = S.workSpace;  sxPx = S.sxPx;  syPx = S.syPx;
        rescaleTarget = S.rescaleTarget;
        x = xw; y = yw;
        files = S.files; H0 = S.rawH; W0 = S.rawW; Hn = S.umH; Wn = S.umW;
        manX = S.manX; manY = S.manY; baseX = S.baseX; baseY = S.baseY;
        colIdx = S.colIdx; rowIdx = S.rowIdx;
        refChID = S.refChID; zPages = S.zPages; zUse_um = S.zUse_um;
        srcPlacement = S.src;
        savedOn = datestr(now,'yyyy-mm-dd HH:MM:SS');
