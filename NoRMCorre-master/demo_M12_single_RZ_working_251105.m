...
name = 'C:\Users\zhang\Desktop\PY\roi3_-650-380-75_38lp_930_x1.8_256x512_3000f_00001.tif';
...
figure;
    ...
    ax1 = subplot(211); hold on; plot(shifts_r(:,1),'--k','linewidth',2); title('displacements along x','fontsize',14,'fontweight','bold')
            set(gca,'Xtick',[])
    ax2 = subplot(212); hold on; plot(shifts_r(:,2),'--k','linewidth',2); title('displacements along y','fontsize',14,'fontweight','bold')
...
%% save rigid-corrected movie (M1) next to input
[folder, base, ext] = fileparts(name);
outname = fullfile(folder, [base '_MC1' ext]);

mc = M1;

% Ensure values are in valid 16-bit range
mc(mc < 0) = 0;
mc(mc > 65535) = 65535;
mc = uint16(mc);
...
for t = 1:T
    if t == 1
        imwrite(mc(:,:,t), outname, 'tif', 'Compression', 'none');
    else
        imwrite(mc(:,:,t), outname, 'tif', 'WriteMode', 'append', 'Compression', 'none');
    end
end

fprintf('Saved rigid-corrected 16-bit movie to:\n%s\n', outname);


%%
[folder, base, ext] = fileparts(name);
outname2 = fullfile(folder, [base '_MC2' ext]);
...
for t = 1:T
    if t == 1
        imwrite(mc2(:,:,t), outname2, 'tif', 'Compression', 'none');
    else
        imwrite(mc2(:,:,t), outname2, 'tif', 'WriteMode', 'append', 'Compression', 'none');
    end
end

fprintf('Saved non rigid-corrected 16-bit movie to:\n%s\n', outname2);
