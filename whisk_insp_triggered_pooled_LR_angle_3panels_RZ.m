...
%   2) inspiration events with a whisk onset within INSP_WHISK_WIN_S
%   3) inspiration events with NO whisk onset within INSP_WHISK_WIN_S
%
...
%   hard envelope threshold = ENV_THR deg
%
% Whisk onset = protraction onset (the event used for grouping):
%   trough of the BP angle = findpeaks(-x), inside the whisk mask, with peak
%   prominence >= PROM_WHISK deg (same trough logic as the inspiration onset).
%   PROM_WHISK is the amplitude gate -- raise it to keep only large whisks.
%
% Breathing:
...
%
% Inspiration event:
%   findpeaks(-brw_bp1_20) -> troughs of the inhale-up breath = inspiration onset
%
% Classification:
%   inspiration with nearby whisk = any L/R whisk onset within INSP_WHISK_WIN_S
%   inspiration without nearby whisk = no L/R whisk onset within INSP_WHISK_WIN_S
%
% Plot:
...
INSP_WHISK_WIN_S = 0.05;  % whisk-nearby window (s)  [NOTE: 0.05 = 50 ms; set 0.10 for the "100 ms rule"]
MIN_WHISK_DIST_S = 0.03;  % minimum distance between detected whisk onsets
PROM_WHISK = 1;           % deg, min prominence of a BP protraction peak to count as a whisk onset
                          % (amplitude gate on onsets; raise to keep only large whisks)
...
for e = 1:numel(sess)
    ...
    try
        ...
        event_trace = brw_bp1_20;  % inhale = positive; inspiration onset = trough
    ...
    tOnL = detect_troughs(xL, tW, m, MIN_WHISK_DIST_S, PROM_WHISK, fpsW);
    tOnR = detect_troughs(xR, tW, m, MIN_WHISK_DIST_S, PROM_WHISK, fpsW);
...
sgtitle(sprintf(['All panels triggered by inspiration: whisk BP %g-%g Hz, breath BP %g-%g Hz, ' ...
                 'no breath inversion, whisk prom>=%g deg, shading = pointwise 95%% CI'], ...
                 WHISK_BP(1), WHISK_BP(2), BREATH_BP(1), BREATH_BP(2), PROM_WHISK));
...
function tOn = detect_troughs(x, t, mask, minDistS, promThrDeg, fps)
% Protraction onset = trough of the BP whisker angle = findpeaks(-x) (start of the
% forward sweep) -- the same trough logic used for the inspiration onset. Restricted
% to the whisk mask; prominence gate promThrDeg (deg) keeps only large whisks;
% troughs kept >= minDistS apart.
    ...
    [~, loc] = findpeaks(-x, ...
        'MinPeakProminence', promThrDeg, ...
        'MinPeakDistance',   max(1, round(minDistS * fps)));

    loc = loc(mask(loc));        % keep only troughs inside whisk epochs
    tOn = t(loc);
end
