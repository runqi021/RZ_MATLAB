% cell_cycle_batch_260831.m
% -----------------------------------------------------------------------
%  per_cell_summary_260812.m with ONE line changed: it renders through
%  cell_cycle_fig_260831 instead of temporal_phase_cell_fig_260812, so the
%  batch produces the PER-CYCLE figure.
%
%  THE CELL IDS ARE THE SAME NUMBERS as the summary batch, because the cell
%  list, the merges (apply_cell_merges_260814) and the active gate are the
%  untouched originals -- only the drawing differs. So Sert_0829_cell331_cycles
%  and Sert_0829_cell331 are the same neuron.
%
%  Output: <rootPath>\figure_260831\<Genotype>_<MMDD>_cell<NNN>_cycles.{png,pdf}
%  The _cycles suffix is what stops it overwriting the summary render.
%
...
addpath(fullfile(repoRoot, 'analysis_260806'));   % pooled_obs / merges / save_avgproj
...
if strcmpi(selMode,'active')
    outDir = fullfile(rootPath, 'figure_260831');
else
    outDir = fullfile(rootPath, 'figure_260831');
end
if ~isempty(ibiMin)
...
for n = 1:numel(idx)
    j = idx(n);  J = CJ(j);
    stem = regexprep(sprintf('%s_%s_cell%03d_cycles', J.group, J.obs(1).recDate, J.cell), ...
                     '[\\/:*?"<>|]', '_');
    ...
    try
        ...
        [figR, projS] = cell_cycle_fig_260831(J.obs, Pr);
...
if isempty(onlyCells)
    idxCsv = fullfile(outDir, 'cell_cycle_index.csv');
else
    idxCsv = fullfile(outDir, 'cell_cycle_index_subset.csv');
end
writetable(T, idxCsv);
