% make_cell_link_260829.m
% -----------------------------------------------------------------------
%  Build a cell_link.mat for the 260829 vagotomised Sert session from its
...
%    roi_match_out_260829\roi_match_curated.{csv,mat}. cell_link_260727.m was
...
%    cell 17 of the curation CSV\..._cell017_pooled*.png, because both read
...
%
%   1. rec_path points into the ARCHIVE (D:\Ventral_surface_summary\Sert\0829\
...
%
%  OUTPUT  D:\Ventral_surface_summary\Sert\0829\cell_pooled\cell_link.mat
%
%  Adapted from make_cell_link_260824.m for the 260829 session.
...
%% ===================== USER-EDITABLE =====================
archiveSession = 'D:\Ventral_surface_summary\Sert\0829';
curatedMat     = fullfile(archiveSession, 'roi_match_out_260829', 'roi_match_curated.mat');
% Needed ONLY to undo the fov_name truncation -- see the block below.
resultsMat     = fullfile(archiveSession, 'roi_match_out_260829', 'roi_match_results.mat');
...
if isfile(outFile) && ~overwrite
    error('make_cell_link_260829:exists', ...
...
%% ---- undo the fov_name truncation -------------------------------------
% roi_match_results.fov_name is fileparts'd, so everything after the first dot
% in a folder name is eaten as an extension:
%     roi12_5x_z-5_13.5lp_3000f_00001  ->  roi12_5x_z-5_13
%     roi14_4.2x_z-5_15lp_3000f_00001  ->  roi14_4
% Five of this session's 24 recordings carry a dot, and the truncated stems match
% no folder on disk, so the archive lookup below fails on exactly those five.
% match.fov_folder keeps the FULL path and is the documented thing to join on
% (never fov_name). The truncated names are kept only to verify the two arrays
% are in the same order before the substitution is trusted.
if isfile(resultsMat)
    RM = load(resultsMat, 'match');
    if isfield(RM,'match') && isfield(RM.match,'fov_folder')
        full = strings(numel(fovs),1);
        for i = 1:numel(fovs)
            [~, bn, ex] = fileparts(char(string(RM.match.fov_folder(i))));
            full(i) = string([bn ex]);          % ex re-appended: the dot is part of the NAME
        end
        agree = arrayfun(@(a,b) startsWith(b, a), fovs, full);
        assert(all(agree), ['fov_folder is not in the same order as fov_name ' ...
            '(%d mismatch) -- refusing to substitute names'], nnz(~agree));
        nFixed = nnz(fovs ~= full);
        if nFixed > 0
            fprintf('fov_name : restored %d truncated name(s) from match.fov_folder\n', nFixed);
            for i = find(fovs ~= full)'
                fprintf('             %-28s -> %s\n', fovs(i), full(i));
            end
        end
        fovs = full;
...
if ~isempty(missing)
    ...
    fprintf(2, '   %s\n', fovs(missing));
    error('make_cell_link_260829:missingRec', ...
...
link.grouping_source = 'roi_match_curated_260829 (reconstructed by make_cell_link_260829)';
link.roi        = R;
...
link.cfg = struct( ...
    ...
    'linkFile',    outFile, ...
    'builtBy',     'make_cell_link_260829.m', ...
    'builtOn',     '2026-08-30', ...
...
if ~isempty(bad)
    ...
    error('make_cell_link_260829:consistency','refusing to write cell_link');
end
