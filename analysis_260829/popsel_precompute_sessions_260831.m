% popsel_precompute_sessions_260831.m
% -----------------------------------------------------------------------
%  popsel_precompute_260816.m, but a group is defined by GENOTYPE + a LIST OF
%  SESSION DATES instead of by genotype alone.
%
%  WHY. popsel_precompute_260816 selects cells with strcmp(p{1}, GROUP), i.e. on
%  the genotype alone, so GROUP='Sert' pools Sert/0721 (intact), Sert/0828 and
%  Sert/0829 (both vagotomised) into one population of 114 -- across the
%  experimental manipulation. Vagotomy is the independent variable here, so the
%  vagotomised sessions need their own group and their own decisions CSV.
%  popsel_decisions_Sert.csv is the INTACT 0721 curation (16/10/22 of 48) and
%  must not be mixed with this one.
%
%  Everything downstream is unchanged: the cache is written as
%  popsel_cache_<GROUP>.mat, so popsel_gui_260816.m opens it by setting its
%  GROUP to the same name. No GUI change is needed.
%
%  Runqi Zhang / 2026-08-31
% -----------------------------------------------------------------------
%
%  --- inherited header from popsel_precompute_260816.m ---
...
% A group is now {name, genotype, {dates}}. The name is what the cache and the
% decisions CSV are called, so it is also what popsel_gui_260816.m's GROUP must
% be set to. dates = {} means every date for that genotype (the old behaviour).
%
% CAVEAT ON 0828, included by RZ decision 2026-08-31. Its 30 registry cells are
% 11+12+7 ROIs from three z-planes (z-5/-10/-15) of ONE site with ZERO
% cross-recording links -- its curation was never turned into a cell_link (see
% project_vagotomized_session_intake_260830). Planes 5 um apart repeat most
% neurons, so a given neuron can enter the population mean up to 3x, and its
% cells cannot be told apart in the GUI by anything except the figure. 0829 is
% properly linked (18 of 36 cells span >1 recording), so the two sessions are
% NOT on the same footing inside this group.
%   To put them on the same footing later: transcribe 0828's curated merges into
%   cell_merge_overrides_260814.m keyed on (recName, roi). That is applied at
%   render time by apply_cell_merges_260814 and renumbers only the merged cells
%   (a merged cell takes min(origCells)), so 0829's ids 331-366 and their
%   figures stay valid. Then rebuild this cache with overwrite = true.
SESSION_GROUPS = { ...
    {'Sert_vagotomized', 'Sert', {'0828','0829'}} ...
    };
GROUPS   = cellfun(@(g) g{1}, SESSION_GROUPS, 'uni', 0);
...
%% ---- which cells ----
% Genotype AND date, so one session's cells cannot leak into another's group.
GENO  = SESSION_GROUPS{gI}{2};
DATES = SESSION_GROUPS{gI}{3};
ids = [];
for c = 1:numel(obsOf)
    if isempty(obsOf{c}), continue; end
    p = regexp(D.OBS(obsOf{c}(1)).label,'/','split');
    if ~strcmp(p{1}, GENO), continue; end
    if ~isempty(DATES) && ~any(strcmp(p{2}, DATES)), continue; end
    ids(end+1) = c; %#ok<SAGROW>
end
if isempty(DATES), dStr = 'all dates'; else, dStr = strjoin(DATES, ', '); end
fprintf('\n%s: genotype %s, session(s) %s -> %d cells in the registry\n', ...
        GROUP, GENO, dStr, numel(ids));
assert(~isempty(ids), '%s selected no cells -- check the genotype and dates', GROUP);
