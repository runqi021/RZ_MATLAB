...
%           peak within +/-MAXLAG; signed lag (L - R), median/IQR/% synchronous.
% 2x2 figure: protraction ITI | retraction ITI | population ITI | L-R lag.
...
fprintf('%d sessions  |  ITI counts: protL %d protR %d\n', ...
...
axis(ax4,'square');
title(ax4, sprintf('L-R protraction peak synchrony  (n=%d)  median %.1f ms, IQR %.1f, sync %.0f%% (|lag|<%.1f ms)', ...
    numel(lag_ms), medianLag, iqrLag, 100*fracSync, sync_ms));
